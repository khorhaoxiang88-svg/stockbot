-- 012 down: remove the 'overdue_issuer_filing' flag code.
--
-- Rows carrying the code cannot survive a rollback, because the constraint they
-- would land under does not permit them. They are DELETED rather than silently
-- rewritten to another code: a flag that says something different from what was
-- computed is worse than no flag. Recomputing after a roll-forward restores
-- them, since risk flags are wholly derived.
...
SELECT security_id, as_of_date, flag_code, severity, evidence_text,
       source_accession, is_unknown
  FROM risk_flags
 WHERE flag_code <> 'overdue_issuer_filing';
